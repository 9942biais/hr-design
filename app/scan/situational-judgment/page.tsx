...
export default function SituationalJudgmentPage() {
  ...
  const { register, handleSubmit, reset, watch, formState: { errors } } = useForm<Record<string, string>>({ defaultValues: state.situational });
  useEffect(() => { if (ready) reset(state.situational); }, [ready, reset]);
  useEffect(() => {
    if (!ready) return;
    const subscription = watch((situational) => update({ situational: situational as Record<string, string> }));
  ...
  return <ScanShell step={3} eyebrow="12개 디자인 상황" title="이 상황에서 어떻게 하겠습니까?">
    <form onSubmit={handleSubmit((situational) => { update({ situational }); router.push("/scan/project"); })} className="space-y-5">
      {situationalQuestions.map((question) => <fieldset key={question.id} className="min-w-0 rounded-3xl bg-white p-6 shadow-card md:p-8">
        <legend className="sr-only">{question.prompt}</legend>
        <div className="min-w-0"><span className="block break-words text-xs font-black uppercase tracking-wider text-violet">{String(question.order).padStart(2,"0")} · {indicators.find((item) => item.id === question.indicatorId)?.name}</span><span className="mt-2 block break-keep text-xl font-black leading-snug">{question.prompt}</span></div>
